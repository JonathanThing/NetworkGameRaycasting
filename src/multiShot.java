...
public class multiShot extends Weapon{
        	...
        	if(reloading == false) {
        		if(this.cooldown < 5) {    	
            		for(int i = 0; i < shots;i++) {
                		...
                		double xComponent = 0;
            			if(i%2 == 1) {
                    		yComponent =  -1*Math.sin((angle.getValue()-0.05*i));
                    		xComponent =  -1*Math.cos((angle.getValue()-0.05*i));	
            			}
            			else if(i%2 ==0) {
                    		yComponent =  -1*Math.sin((angle.getValue()-0.05*i));
                    		xComponent =  -1*Math.cos((angle.getValue()-0.05*i));		            				
            			...
            			projectilesList.add(new Projectile(new Vector(playerPos.getX(),playerPos.getY()), 20, 20, "Bullet", angle, fireball, 10, 10, 0, 1, xComponent, yComponent));
            		}
                }
	...
	multiShot(double x, double y, int width, int height, String name, TextureManager sprite, double damage, double fireRate, 
